...
var __gong__map_Predecessor = make(map[string]*Predecessor)
	...
	for _, expr := range assignStmt.Rhs {
				...
				case *ast.Ident:
					...
					case "Project":
						...
						case "Tasks":
							...
							target := __gong__map_Task[targetIdentifier]
							__gong__map_Project[identifier].Tasks =
								append(__gong__map_Project[identifier].Tasks, target)
						}
					case "Task":
						switch fieldName {
						// insertion point for slice of pointers assign code
						case "Predecessors":
							// remove first and last char
							targetIdentifier := ident.Name
							target := __gong__map_Predecessor[targetIdentifier]
							__gong__map_Task[identifier].Predecessors =
								append(__gong__map_Task[identifier].Predecessors, target)
						}
					}
				case *ast.SelectorExpr:
					slcExpr := arg
		...
		case *ast.BasicLit, *ast.UnaryExpr:
			...
			case "Predecessor":
				...
				case "Name":
					...
					__gong__map_Predecessor[identifier].Name = fielValue
				}
			case "Project":
				switch fieldName {
				// insertion point for field dependant code
		...
		case *ast.Ident:
			...
			case "Predecessor":
				...
				case "Task":
					targetIdentifier := ident.Name
					__gong__map_Predecessor[identifier].Task = __gong__map_Task[targetIdentifier]
				}
			case "Project":
				switch fieldName {
				// insertion point for field dependant code
		...
		case *ast.SelectorExpr:
			...
			if Sel := selectorExpr.Sel; Sel != nil {
				...
				case "Predecessor":
					...
					case "DependencyType":
						var val DependencyType
						err := (&val).FromCodeString(enumValue)
						if err != nil {
							log.Fatalln(err)
						}
						__gong__map_Predecessor[identifier].DependencyType = DependencyType(val)
					}
				case "Project":
					switch fieldName {
					// insertion point for enum assign code
