...
func fillUpTree(
	...
	for _, gongStruct := range sliceOfGongStructsSorted {
		...
		case "Predecessor":
			...
			set := *models.GetGongstructInstancesSet[models.Predecessor](probe.stageOfInterest)
			for _predecessor := range set {
				nodeInstance := (&tree.Node{Name: _predecessor.GetName()}).Stage(probe.treeStage)
				nodeInstance.IsNodeClickable = true
				nodeInstance.Impl = NewInstanceNodeCallback(_predecessor, "Predecessor", probe)

				nodeGongstruct.Children = append(nodeGongstruct.Children, nodeInstance)
			}
		case "Project":
			nodeGongstruct.Name = name
			set := *models.GetGongstructInstancesSet[models.Project](probe.stageOfInterest)
			for _project := range set {
				nodeInstance := (&tree.Node{Name: _project.GetName()}).Stage(probe.treeStage)
				nodeInstance.IsNodeClickable = true
		...
		case "Task":
			nodeGongstruct.Name = name
			set := *models.GetGongstructInstancesSet[models.Task](probe.stageOfInterest)
			for _task := range set {
				nodeInstance := (&tree.Node{Name: _task.GetName()}).Stage(probe.treeStage)
				nodeInstance.IsNodeClickable = true
